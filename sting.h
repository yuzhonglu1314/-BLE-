...
extern _ARMABI void *memset(void * /*s*/, int /*c*/, size_t /*n*/) __attribute__((__nonnull__(1)));
    ...
    */
		//memest 记忆集
		/* *将c的值（转换为无符号字符）复制到s所指向的对象的*前n个字符中的每个字符中。 *返回：s的值。 */
		
		//memset函数：实现对某内存块的各字节单元整体赋同样的值
    //void *memset (void *s, char ch, unsigned n) → 将数组a的每个数据单元赋值为0 memset (a, 0,10*sizeof(int));
    //如何引用一个已经定义过的全局变量？ ?
		//答：extern ?可以用引用头文件的方式，也可以用extern关键字，如果用引用头文件方式来引用某个在头文件中声明的全局变理
		//假定你将那个编写错了，那么在编译期间会报错，如果你用extern方式引用时，
    ...
    //extern 关键字的作用
		...
		//2. 用于extern “c
		//extern “c”的作用就是为了能够正确实现C++代码调用其他C语言代码。加上extern "C"后，
		//会指示编译器这部分代码按C语言的编译方式进行编译，而不是C++的。
